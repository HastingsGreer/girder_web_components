<template lang="pug">
v-card.data-details(v-if="data.length", flat, :style="{ width: width + 'px' }")
  v-toolbar(card, dark, dense, color="primary")
    v-toolbar-title.details-title
      v-icon.pr-2.mdi-18px {{ icon }}
      | {{ title }}
    v-spacer
    v-dialog(v-if="datum", full-width, max-width="800px")
    ...
    v-layout(slot="row", slot-scope="props", column)
      v-flex
        v-btn.ma-0.pl-0(flat, small, block,
            ...
            @click="props.datum.handler(data)")
          v-icon {{ $vuetify.icons[props.datum.icon] }}
          | {{ props.datum.name }}
</template>
...
export default {
  ...
  computed: {
    title() {
      return this.datum ? (this.datum.name || this.datum.login) : `${this.data.length} Selections`;
    },
    datum() {
